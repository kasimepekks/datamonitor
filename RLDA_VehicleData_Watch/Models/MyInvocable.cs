...
namespace RLDA_VehicleData_Watch.Models
{
    ...
    //                        _hubContext.Clients.Group(MyHub.user).SendAsync("ReloadDataACC", _vehicleID, name, acctimedomainresult, accstatisticresult);
    //                        _hubContext.Clients.Group(MyHub.user).SendAsync("SpeedtoDistance", _vehicleID, vehiclecumdistance[_vehicleID], speed, brake, Lat, Lon, zerotime);//zerotime用来初始化每次的开始时间，每当有新数据读取时，zerotime初始化为0，传入前端，用于前端的speed和brake仪表盘的显示
    ...
    //                        _hubContext.Clients.Groups(MyHub.user).SendAsync("ReloadDataWFT", _vehicleID, name, timedomainresult, statisticresult);
    //                    }
    //                });
